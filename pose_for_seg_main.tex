...
In this paper, we present a unified framework for performing camera pose estimation and semantic parsing with a 3D semantic map and deep networks, for the applications of visual-based autonomous navigation. The algorithm is simple and run efficient while yielding strong results in both self-localization and backgrounds parsing. More importantly, the two information are mutually beneficial, where pose helps give good priori for segmentation and semantic guides a better localization.
...
For algorithm, we would like to simplify the 3D point cloud map with 3D models of road and buildings, based on which a better and faster renderring can be performed.
