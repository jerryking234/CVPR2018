...
% <<<<<<< HEAD
In this paper, we present a unified framework for performing camera pose estimation and semantic parsing with 3D semantic map and deep networks for online videos, for the applications of visual-based outdoor robotic navigation. The algorithm is simple and can run efficient, meanwhile yields strong results in both self-localization and scene parsing. More importantly, we show that the two information are actually mutually beneficial in our system, where pose helps give good priori for segmentation and semantic guides a better localization. Finally, since semantics vary from one scene to another, it is infeasible to use a general approach to train a model each scene, or a single model with millions of classes. However, based on the system, as long as a 3D semantic model is built, pose estimation can be performed, and images can be parsed accordingly, which we will nail at later.

In the near future, we plan to extend in several aspects. In terms of dataset, additional to a relative small data with few objects used in this paper, yielding limited segment model for object, significantly larger and harder data will be collected.
Besides dense point cloud, we would like to simplify the 3D point cloud map with 3D models of road and buildings, based on which a better and faster projection rendering can be performed. 
