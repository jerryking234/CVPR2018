...
In order to remove those fake object points (circled in blue at \figref{fig:data}(b)), thanks to the fact that the 3D world is recorded multiple rounds, and each round has a full coverage of the 3D map, we can use point clouds consistency to handle the issue.
